...
%% 3. DAUGMAN — IRIDE ─────────────────────────────────────────────────────────
% Il centro dell'iride coincide con quello della pupilla:
% la finestra di ricerca è ristretta attorno al centro già trovato.
sr      = 5;
...
% 72 punti equi-spaziati → passo angolare di 5°
    ...
    for cy = y_range
        ...
        for k = 1:Nr
            ...
            if mean(valid) < 0.8
                ...
                mean_I(k) = mean(I(idx));   % ≈ (1/2πr) ∮ I ds
            end
        end

        if any(isnan(mean_I)), continue; end

        dI_dr = abs(diff(mean_I));
